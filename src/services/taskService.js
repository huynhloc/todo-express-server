...
export function getTask(req, res) {
  ...
  if (!taskId) {
    ...
    Task.findById(taskId)
      .where('flg').ne(ACTION_FLG.DELETED)
      .select({ __v: 0, flg: 0 }).exec().then(task => {
...
export function getTasks(req, res) {
  ...
  const query = Task.find(cond)
    .select({ __v: 0, flg: 0 })
    .sort('-createdAt')
    .limit(pageSize)
  if (page !== 0) query.skip(page * pageSize)

  Promise.all([countQuery.exec(), query.exec()]).then(result => {
...
export function deleteTask(req, res) {
  ...
  if (!taskId) {
    ...
    Task.findByIdAndUpdate(taskId, { $set: { flg: ACTION_FLG.DELETED } }, { new: true })
      .select('_id title createdAt').exec().then(task => {
        if (task) {
          return res.status(200).json({
            message: 'Delete Successful',
            task
